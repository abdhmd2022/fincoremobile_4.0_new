...
class ItemsTotalClickedCostCenter extends StatefulWidget {
  ...
  @override
  _ItemsTotalClickedCostCenterPageState createState() =>
...
class _ItemsTotalClickedCostCenterPageState
    extends State<ItemsTotalClickedCostCenter>
  ...
  List<String> spinner_list = ['Ledger', 'Bills', 'Voucher Type'];
  ...
  Future<void> generateAndSharePDF_Ledger() async {
    ...
    for (int i = 0; i < pageCount; i++) {
      ...
      final rows = subset
          .map(
            (item) => [item.Partyledger, item.qty, formatAmount(item.amount)],
          )
          .toList();
        ...
        pw.Page(
          build: (_) => pw.Column(
            ...
            children: [
              ...
              pw.SizedBox(height: 10),
              pw.Text(
                'Cost Center: ${formatCostCenter(cost_center)}',
                style: pw.TextStyle(
                  fontSize: 16,
                  fontWeight: pw.FontWeight.bold,
                ),
  ...
  Future<void> generateAndShareCSV_VchType() async {
    ...
    final csvData = [
      ...
      ...vouchertype_list.map(
        (e) => [e.vchname, e.qty, formatAmount(e.amount)],
      ),
    ];
